...
def main():
    ...
    parser.add_argument("--mab_memory_path", default="mab_preferences_unix.json", type=str,
                        help="MAB记忆文件保存路径")
    ...
    # 创建Model实例，与训练时完全一致
    from model import Model
    model_tgt = Model(encoder, config, tokenizer, args)
    ...
    if args.transfer_memory_path and args.transfer_memory_path.endswith('.json'):
        ...
        # 假设transfer_memory_path1和transfer_memory_path2分别为两个模型的memory
    ...
    for index, example in enumerate(eval_dataset):
        ...
        if adv_code is None:
            ...
            if pred_orig == label.item():
                ...
                pair_id += 1
                print(f"Collecting code pair for sample {index}")
    ...
    print("unixAdversarial examples saved.")
    # --------------------------------------------------------------------------------------------
    ...
    print(f"Total code pairs collected: {len(code_pairs)}")
