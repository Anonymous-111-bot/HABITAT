...
from transformers import (RobertaForMaskedLM, RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer,
                          RobertaModel, T5ForConditionalGeneration, T5Config)
...
def main():
    ...
    parser.add_argument("--mab_memory_path", default="mab_preferences_cod5.json", type=str)
    ...
    # 创建Model实例，与训练时完全一致
    from model import Model as CodeT5
    model_tgt = CodeT5(encoder, config, tokenizer, args)
            ...
            for key in state_dict.keys():
                if key in model_dict:
                    if state_dict[key].shape == model_dict[key].shape:
                        ...
                        print(
                            f"  Skipping {key} due to shape mismatch: {state_dict[key].shape} vs {model_dict[key].shape}")
                else:
    ...
    # 测试替代模型的前向传播（只用encoder）
    try:
        test_input = torch.randint(0, 1000, (1, 10)).to(args.device)
        with torch.no_grad():
            test_output = model_sub.encoder(input_ids=test_input)
        print(f"✓ Substitute model encoder forward pass successful")
    ...
    for index, example in enumerate(eval_dataset):
        ...
        if memory_manager1 is not None and memory_manager2 is not None:
            adv_code = attacker.stacked_transfer_attack(source_code, label, memory_manager1, memory_manager2,
                                                        example_index=index, strategy="adaptive")
        ...
        if adv_code is None:
            ...
            wrong += 1
            results['attack_success'].append(None)
        elif adv_code:
    ...
    print("t5Adversarial examples saved.")
    # --------------------------------------------------------------------------------------------
