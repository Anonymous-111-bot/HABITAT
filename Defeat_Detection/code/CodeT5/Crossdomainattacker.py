...
class CrossDomainAttacker:
    ...
    def _calculate_loss(self, orig_features, new_features):
        ...
        with torch.no_grad():
            # 只用T5的encoder部分
            orig_outputs = self.model_sub.encoder(
                input_ids=torch.tensor([orig_features.input_ids]).to(self.args.device))
            new_outputs = self.model_sub.encoder(input_ids=torch.tensor([new_features.input_ids]).to(self.args.device))

            orig_embeddings = orig_outputs.last_hidden_state
