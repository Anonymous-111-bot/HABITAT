...
class Model(nn.Module):
    ...
    def forward(self, input_ids=None, labels=None):
        ...
        # 如果序列长度小于block_size，需要padding
        if seq_len < self.args.block_size:
            # 计算需要padding的长度
            pad_len = self.args.block_size - seq_len
            # 创建padding张量
            pad_tensor = torch.full((batch_size, pad_len), self.tokenizer.pad_token_id, 
                                  dtype=input_ids.dtype, device=input_ids.device)
            # 拼接原始输入和padding
            input_ids = torch.cat([input_ids, pad_tensor], dim=1)
        elif seq_len > self.args.block_size:
            # 如果序列长度大于block_size，截断到block_size
