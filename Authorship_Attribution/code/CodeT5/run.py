...
from model import Model as CodeT5, CodeT5_for_sim, CodeT5_sub
...
def similarity(args, model, tokenizer, config):
    ...
    list_distance = []
    try:
        model_sub = CodeT5_sub(T5ForConditionalGeneration.from_pretrained("Salesforce/codet5-base"), config, tokenizer,
                               args).to(args.device)
    except:
        model_sub = CodeT5_sub(T5ForConditionalGeneration.from_pretrained('../cache/Salesforce/codet5-base/'), config,
                               tokenizer, args).to(args.device)
    ...
    for i, (example) in enumerate(eval_dataloader):
        ...
        if pred_label == label:
            ...
            similarity = torch.cosine_similarity(torch.flatten(pre_trained_embedding),
                                                 torch.flatten(fine_tuned_embedding), dim=0)
            list_similarity.append(similarity.item())
            list_distance.append(
                distance(torch.flatten(pre_trained_embedding), torch.flatten(fine_tuned_embedding)).item())
...
def main():
    ...
    encoder = model_class.from_pretrained('Salesforce/codet5-base')
