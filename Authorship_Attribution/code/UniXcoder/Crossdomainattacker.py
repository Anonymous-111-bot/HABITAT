...
class UCB1MAB:
    ...
    def select_arm(self) -> int:
        ...
        for arm in range(self.num_arms):
            ...
            exploration = self.alpha * np.sqrt(2 * np.log(self.total_pulls) / self.counts[arm])
...
class CrossDomainAttacker:
    ...
    def __init__(self, args, model_sub, model_tgt, tokenizer, model_mlm, tokenizer_mlm,
        ...
        self.code_transformations = [
            "if False:\n    unused_var_{} = 'hello world!'".format(random.randint(1000, 9999)),
            "while False:\n    unused_var_{} = 'unreachable'\n    break".format(random.randint(1000, 9999)),
            "unused_var_{} = {}".format(random.randint(1000, 9999),
                                        random.choice(["True", "False", "''", "[]", "{}", "0"])),
            "def unused_func_{}():\n    return None".format(random.randint(1000, 9999)),
    ...
    def _get_prediction(self, code, label):
        ...
        with torch.no_grad():
            logits = self.model_tgt(torch.tensor([features.input_ids]).to(self.args.device))
    ...
    def _calculate_loss(self, orig_features, new_features):
        ...
        # 提取特征向量
        with torch.no_grad():
            orig_embeddings = self.model_sub(torch.tensor([orig_features.input_ids]).to(self.args.device))[0]
            new_embeddings = self.model_sub(torch.tensor([new_features.input_ids]).to(self.args.device))[0]

        # 计算距离
        distance = torch.norm(orig_embeddings - new_embeddings)
    ...
    def _find_safe_insertion_lines(self, code_lines):
        ...
        for i, line in enumerate(code_lines):
            ...
            # 在多行字符串或注释中，跳过
            if in_multiline_string or in_multiline_comment:
                continue
    ...
    def _find_important_positions(self, code, top_k):
        ...
        for pos in safe_lines:
            try:
                ...
                position_scores.append((pos, 0.0))
    ...
    def MAB_attack(self, source_code, label):
        ...
        for iteration in range(exploration_iterations):
            ...
            for pos in sorted(selected_transforms.keys(), reverse=True):
                ...
                try:
                    ...
                    modified_lines.insert(pos, transform_code)
                except Exception as e:
        ...
        for iteration in range(exploration_iterations, total_iterations):
            ...
            if reward > best_reward:
                ...
                attack_success = current_success
            if current_success:
                logger.info(f"Attack succeeded during MAB-guided search! Iteration {iteration - exploration_iterations + 1}")
                return transformed_code
        logger.info(f"Reached maximum iterations. Attack {'succeeded' if attack_success else 'failed'}. Returning best code.")
        return best_code
    ...
    def adapt_transfer_attack(self, source_code, label, memory_manager, example_index=None):
        ...
        for iteration in range(random_iterations, total_iterations):
            ...
            # 如果攻击成功，提前结束
            if current_success:
                logger.info(f"在MAB引导搜索阶段攻击成功！迭代 {iteration + 1}")
                return transformed_code
