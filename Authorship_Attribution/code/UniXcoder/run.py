...
MODEL_CLASSES = {
    ...
    'codellama': (LlamaConfig, LlamaForCausalLM, LlamaTokenizer),
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        ...
        code_pairs_file_path = os.path.join(folder, f'cached_{file_type}.pkl')
        ...
        try:
            ...
            logger.info("从缓存文件 %s 加载特征", cache_file_path)

        except:
            logger.info("在 %s 创建数据集文件的特征", file_path)
            code_files = []
            with open(file_path, encoding='utf-8') as f:
                for line in f:
                    if ' <CODESPLIT> ' in line:
                        ...
                        code_files.append(code_snippet)
            # 保存处理后的样本
            with open(code_pairs_file_path, 'wb') as f:
                pickle.dump(code_files, f)
            logger.info("将特征保存到缓存文件 %s 中", cache_file_path)
        ...
        if 'train' in file_path:
            for idx, example in enumerate(self.examples[:3]):
                ...
                logger.info("input_tokens: {}".format([x.replace('\u0120', '_') for x in example['input_token']]))
    ...
    def __getitem__(self, item):
        ...
        if isinstance(input_ids, list):
            ...
            input_ids = input_ids.long().view(-1)
        else:
            raise ValueError(f"Unexpected type for input_ids: {type(input_ids)}")
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['eval_accuracy'] >= best_f1:
                        ...
                        try:
                            # 创建保存目录
                            checkpoint_prefix = 'checkpoint_last-f1'
                            output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))

                            # 确保目录存在，添加 exist_ok=True 避免目录已存在时报错
                            os.makedirs(output_dir, exist_ok=True)

                            model_to_save = model.module if hasattr(model, 'module') else model
                            ft = '_ft' if args.ft else ''
                            output_file = os.path.join(output_dir, 'model{}.bin'.format(ft))

                            # 保存模型
                            torch.save(model_to_save.state_dict(), output_file)
                            logger.info("Saving model checkpoint to %s", output_file)
                        except Exception as e:
                            logger.error(f"Error saving model checkpoint: {str(e)}")
                            # 尝试备用路径
                            try:
                                # 使用绝对路径
                                abs_output_dir = os.path.abspath(os.path.join(os.getcwd(), "model_checkpoints"))
                                os.makedirs(abs_output_dir, exist_ok=True)
                                backup_file = os.path.join(abs_output_dir, f"model{ft}_backup.bin")
                                torch.save(model_to_save.state_dict(), backup_file)
                                logger.info(f"Saved model to backup location: {backup_file}")
                            except Exception as e2:
                                logger.error(f"Failed to save model to backup location: {str(e2)}")
...
def main():
    ...
    config = config_class.from_pretrained('/root/autodl-tmp')
    config.num_labels = args.number_labels
    tokenizer = tokenizer_class.from_pretrained('/root/autodl-tmp')

    if args.block_size <= 0:
        args.block_size = tokenizer.max_len_single_sentence  # Our input block size will be the max possible for the model
        args.block_size = min(args.block_size, tokenizer.max_len_single_sentence)
    if args.model_name_or_path:
        encoder = model_class.from_pretrained('/root/autodl-tmp')
    ...
    if args.do_train:
        ...
        # ------------------------------------------------------------
        checkpoint_prefix = 'checkpoint_last-f1/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        ft = '_ft' if args.ft else ''
        checkpoint_prefix = 'checkpoint_last-f1/model{}.bin'.format(ft)
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
    ...
    if args.do_asr and args.local_rank in [-1, 0]:
        # Clear GPU cache before ASR evaluation
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    # Similarity function with memory optimization and proper error handling
    if args.do_sim and args.local_rank in [-1, 0]:
        try:
            # Function placeholder for similarity calculations
            # This is a stub that will be implemented as needed

            def similarity(args, model, tokenizer):
                logger.info("Running similarity calculation")
                # Actual implementation would go here
                return {"similarity": "Not implemented"}

            # Clear GPU cache before similarity evaluation
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            ...
            # Continue execution even if similarity fails
