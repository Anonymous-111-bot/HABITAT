...
from Authorship_Attribution.code.CodeBERT.model import Model, Model_sub, Model_for_sim
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        ...
        file_type = os.path.splitext(os.path.basename(file_path))[0]
        ...
        try:
            ...
            # 不管缓存的格式如何，统一转换为一种简单的格式：(input_ids, label)对
            self.examples = []

            # 检查缓存数据类型并提取特征
            if isinstance(cached_data, list):
                for item in cached_data:
                    # 根据不同的缓存格式提取input_ids和label
                    if isinstance(item, dict):
                        input_ids = item['input_ids']
                        label = item['label']
                    elif hasattr(item, 'input_ids') and hasattr(item, 'label'):
                        input_ids = item.input_ids
                        label = item.label
                    else:
                        # 尝试其他可能的属性名
                        try:
                            input_ids = getattr(item, 'input_ids', getattr(item, 'source_ids', None))
                            if input_ids is None:
                                # 最后尝试作为元组/列表访问
                                input_ids = item[0] if isinstance(item, (list, tuple)) else item

                            label = getattr(item, 'label', None)
                            if label is None:
                                label = item[1] if isinstance(item, (list, tuple)) and len(item) > 1 else 0
                        except:
                            raise ValueError(f"无法从缓存项提取特征: {type(item)}")

                    # 将提取的特征添加到examples中
                    self.examples.append((input_ids, label))

            logger.info(f"从缓存文件 {cache_file_path} 加载了 {len(self.examples)} 个特征")

        except Exception as e:
            logger.info(f"加载缓存时出错: {e}")
            logger.info(f"从文件 {file_path} 创建新的特征")

            # 这里是您原始的代码，用于从文件创建特征
            with open(file_path, encoding='utf-8') as f:
                for line in f:
                    if ' <CODESPLIT> ' in line:
                        code_snippet, label = line.strip().split(' <CODESPLIT> ')
                        ...
                        # 将特征添加到examples中
                        self.examples.append((inputs['input_ids'], int(label)))

            # 保存缓存文件 - 使用简单的(input_ids, label)对格式
            torch.save(self.examples, cache_file_path)
            logger.info(f"将 {len(self.examples)} 个特征保存到缓存文件 {cache_file_path}")

    def __len__(self):
        return len(self.examples)

    def __getitem__(self, item):
        input_ids, label = self.examples[item]
...
def pytest_addoption(parser):
    parser.addoption("--args", action="store", default="default_value")
        ...
        for step, batch in enumerate(bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    # 在您的训练代码的适当位置，比如在验证集上取得最佳性能时，保存模型和分词器
                    if results['eval_accuracy'] >= best_f1:
                        ...
                        output_dir = os.path.join(output_dir, 'model_finallyly{}.bin'.format(ft))
...
def evaluate(args, model, tokenizer, prefix="", pool=None, eval_when_training=False):
    if use_unixcoder_cache:
        eval_dataset = load_unixcoder_cache_for_codebert(args, tokenizer, evaluate=True)
    else:
    ...
    for batch in tqdm(eval_dataloader):
        ...
        labels = batch[1].to(args.device)
        with torch.no_grad():
            lm_loss, logit = model(inputs, labels, use_adapter=use_unixcoder_cache)
...
def tesr(args, model, tokenizer, prefix="", pool=None, best_threshold=0):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
...
model_path = './output_dir/t5/model_t5.bin'
...
path = os.path.join('./saved_models', 'gcjpy', 'checkpoint-best-f1', 'model——.bin')
...
def ASR(args, model, tokenizer):
    ...
    for i, (example_eval) in enumerate(eval_dataloader):
        ...
        if np.argmax(eval_logit) == eval_label:
            total_num += 1
            adv_inputs_id = torch.tensor(
                convert_examples_to_features(adv_codes[i], eval_label, tokenizer, args).input_ids).unsqueeze(0).to(
                args.device)
...
def similarity(args, model, tokenizer):
    ...
    if args.pretrained_encoder == 'codebert':
        ...
        model_sub = gcb_Model_sub(RobertaForSequenceClassification.from_pretrained("microsoft/graphcodebert-base")).to(
            args.device)
    ...
    for i, (example_eval) in enumerate(eval_dataloader):
        ...
        if np.argmax(eval_logit) == eval_label:
            ...
            similarity = torch.cosine_similarity(torch.flatten(pre_trained_embedding[0]),
                                                 torch.flatten(fine_tuned_embedding[0]), dim=0)
            # print('similarity.shape: ', similarity.shape)
            list_similarity.append(similarity.item())
            list_distance.append(
                distance(torch.flatten(pre_trained_embedding[0]), torch.flatten(fine_tuned_embedding[0])).item())
...
def main():
    ...
    if args.do_train:
        ...
        # ------------------------------------------------------------
        checkpoint_prefix = 'save_model/model——.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        model.load_state_dict(torch.load(output_dir))
        model.to(args.device)
        tesr(args, model, tokenizer, pool=pool, best_threshold=0.5)
